...
test:
	...
	# Create profiles
	go tool pprof -pdf perf.test prof_cpu.out > result_cpu.pdf
	go tool pprof -pdf perf.test profile_block.out > result_block.pdf
	go tool pprof -pdf perf.test prof_mem.out > result_mem.pdf
	...
	go-torch perf.test prof_cpu.out --file=result_cpu_flames.svg
